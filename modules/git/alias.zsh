...
zstyle -s ':prezto:module:git:log:medium' format '_git_log_medium_format' \
  || _git_log_medium_format='%C(bold)Commit:%C(reset) %C(yellow)%H%C(auto)%d%n%C(bold)Author:%C(reset)        %C(cyan)%an <%ae>%n%C(bold)AuthorDate:%C(reset)    %C(dim white)%ai (%ar)%C(reset)%n%C(bold)Committer:%C(reset)     %C(cyan)%cn <%ce>%n%C(bold)CommitterDate:%C(reset) %C(dim white)%ci (%cr)%C(reset)%n%+B'
zstyle -s ':prezto:module:git:log:oneline' format '_git_log_oneline_format' \
  || _git_log_oneline_format='%C(yellow)%h%C(reset) %C(dim white)(%ar)%C(reset) %s%C(auto)%d%C(reset)'
zstyle -s ':prezto:module:git:log:brief' format '_git_log_brief_format' \
  || _git_log_brief_format='%C(yellow)%h%C(reset) %s%n%C(cyan)%an %C(dim white)(%ar)%C(auto)%d%C(reset)%n'
...
if ! zstyle -t ':prezto:module:git:alias' skip 'yes'; then
  ...
  alias gbpg='git branch --verbose | grep -v "master" | grep -v "\*" | grep "\[gone\]" | awk "{print \$1}" | xargs -n 1 git branch -D'
  alias gbpm='git branch --merged | grep -v "master" | grep -v "\*" | xargs -n 1 git branch -d'
  ...
  alias glo='git log --topo-order --pretty=tformat:"${_git_log_oneline_format}"'
  alias glg='git log --topo-order --all --graph --pretty=format:"${_git_log_oneline_format}"'
  alias glgb='git log --topo-order --all --graph --pretty=format:${_git_log_brief_format}'
